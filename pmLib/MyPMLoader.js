function MyPMLoader(url,LODArray,camera,animationType,animationSpeed){
    ...
    this.load=function(url,LODArray,camera, onLoad, onProgress, onError)
    {
        ...
        //animLoader.load(url + '/gltf/childFemale_crawl.glb', function (gltfScene)
        {
    ...
    {//parse的意思是解析，只被load函数调用一次
        ...
        {//index:0-330   lengthindex:331
            ...
            //旧的THREE JS不光无法生成实例化对象也无法渲染实例化对象，因此必须看一下如何将新版的THREE JS与引擎相结合
            /*if(index==0){//开启实例化渲染的代码后用于实例化的那个模型骨骼绑定出现了问题
                var scene=appInst._renderScenePass.scene;//window中含有scene对象//0000
                ...
                //var material=new THREE.MeshStandardMaterial();//material.map=mesh[Meshid].material.map;
                material.map=new THREE.TextureLoader().load( 'myModel/dongshizhang5/Texture_0.jpeg' );

                //console.log(material);
                //geometry.computeVertexNormals();
                //geometry.computeFaceNormals();

                var mesh2=new THREE0.InstancedMesh(geometry,material,3);
